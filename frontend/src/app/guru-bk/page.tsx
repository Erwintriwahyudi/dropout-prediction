...
export default function GuruBKPage() {
  const [data, setData] = useState<GuruBK[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<GuruBK | null>(null);
  ...
  const openAddModal = () => {
    setEditingItem(null);
    setFormData({ username: "", password: "", nama: "", nip: "" });
    setShowModal(true);
  };

  const openEditModal = (item: GuruBK) => {
    setEditingItem(item);
    setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        ...
        // Add mode
        await api.post("/guru-bk/", formData);
      }
      setShowModal(false);
      fetchData();
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-950 p-8">
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-white">Guru BK</h1>
            <p className="text-gray-400 mt-2">Data Guru Bimbingan Konseling yang terdaftar di sistem.</p>
          </div>
          <button 
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
              <div className="p-4 border-b border-gray-800 flex items-center justify-between">
                <h2 className="text-lg font-semibold text-white">
                  {editingItem ? "Edit Guru BK" : "Tambah Guru BK"}
                </h2>
